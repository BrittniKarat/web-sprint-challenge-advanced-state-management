...
1. What problem does the context API help solve?
    - context API helps create dryer code by getting rid of prop drilling

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  - the store helps keep our global state in a centralized location accessible by any part of our app. It allows for pulling just the state needed, as needed.
  - reducers cluster hold like pieces of state that are meant to be used together.
  - actions use the type and, optionally, payload properties that are dispatched to tell the reducer how to update the state

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
  - Thunk is a piece of middleware that middleman the process between action creators and reducers. They can forward an action untouched, stop actions from being dispatched, and/or dispatch multiple actions as necessary.
